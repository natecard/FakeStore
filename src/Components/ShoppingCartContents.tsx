...
export default function ShoppingCartContents(props: item) {
  return (
    ...
    >
      <img
        className=" rounded-md flex md:h-44 justify-end self-center"
        src={props.image}
        alt={props.title}
      />
      <h2 className="text-center self-center truncate">{props.title}</h2>
      <div className="justify-center  self-center flex flex-row md:p-2">
        <input
          onChange={() => props.handleCartChange(props, props.quantity, event)}
          className="h-7 md:p-1 md:pl-1.5 md:w-14 hover:border-green-700 focus:border-green-700 focus:outline-none border-2 border-[#f3f3f3] hover:border-2 focus:border-2 border-solid rounded-md bg-[#f3f3f3]"
      ...
      <div className="flex flex-row mb-2 self-center">
        <button
          ...
          className="uppercase md:p-1 bg-black rounded hover:bg-white hover:text-black text-white font-bold"
        >
          Remove from cart
        </button>
      </div>
